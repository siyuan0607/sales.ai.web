...
<div class="app-container">
    <el-form :inline="true" :model="searchForm" class="demo-form-inline">
        ...
        <el-form-item label="预约日期">
            <el-date-picker v-model="searchForm.appoint_date" type="daterange" unlink-panels range-separator="—" start-placeholder="开始日期" end-placeholder="结束日期" />
        </el-form-item>
        <el-form-item label="生命周期">
            <el-select v-model="searchForm.life_cycle" placeholder="请选择">
                <el-option v-for="item in leftCycleOptions" :key="item.value" :value="item.value" :label="item.label"></el-option>
            </el-select>
        </el-form-item>
        <el-form-item label="客群类型">
            <el-select v-model="searchForm.customer_type" placeholder="请选择">
                <el-option v-for="item in customerTypeOptions" :key="item.key" :value="item.value" :label="item.key"></el-option>
            </el-select>
        </el-form-item>
    ...
    <el-table v-loading="listLoading" :data="list" style="width: 100%" element-loading-text="Loading" border fit highlight-current-row>
        ...
        <el-table-column prop="life_cycle" label="生命周期" width="100" align="center"></el-table-column>
        <el-table-column prop="customer_type" label="客群类型" width="100" align="center"></el-table-column>
    ...
    <div class="example-pagination-block">
        <el-pagination hide-on-single-page align="center" layout="prev, pager, next" :page-size="pageSize" v-model:current-page="page" :total="total" @current-change="handleCurrentChange" />
    </div>
    ...
    methods: {
        ...
        search() {
            ...
            if (this.searchForm.appoint_date.length === 2) {
                this.fetchParam.appoint_date_start = this.searchForm.appoint_date[0].toLocaleDateString() + ' 00:00:00'
                this.fetchParam.appoint_date_end = this.searchForm.appoint_date[1].toLocaleDateString() + ' 23:59:59'
